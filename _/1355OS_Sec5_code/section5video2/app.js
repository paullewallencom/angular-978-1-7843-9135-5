...
.controller('VideoPlayerController', function($scope, $q, $http) {
  ...
  var videoId = $http.jsonp('https://api.twitch.tv/kraken/videos/top?game=Gaming+Talk+Shows&period=month&callback=JSON_CALLBACK').then(function(response) {
    $scope.status.push('Video details loaded');
